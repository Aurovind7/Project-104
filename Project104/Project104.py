...
for i in range(len(file_data)):
    ...
    new_data_mean.append(float(number))
    new_data_median.append(float(number))
    new_data_mode.append(float(number))

n_mean = len(new_data_mean)
n_median = len(new_data_median)

new_data_median.sort()

# MEAN
total = 0

for x in new_data_mean:
    total += x
...
# MEDIAN
if n_median % 2 == 0:
    median1 = float(new_data_median[n_median//2])

    median2 = float(new_data_median[n_median//2 - 1])

    median = (median1 + median2)/2

else:
    median = new_data_median[n_median//2]

print("The Median is: "+str(median))

# MODE

data= Counter(new_data_mode)
mode_data_for_range = {
                        "75-85" : 0,
                        "85-95" : 0,
                        "95-105" : 0,
